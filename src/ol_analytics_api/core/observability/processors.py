...
Ported verbatim from mitol-django-observability
(ol-django:src/observability/mitol/observability/processors.py) — this module
has no Django dependency, so it's reused as-is rather than reinvented. Keeping
the exact same trace_id/span_id/pod/namespace field names here means logs
from this service correlate with mitxonline/mit-learn/learn-ai logs in Loki
using the same query patterns.
"""
