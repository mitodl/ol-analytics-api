...
class LearnerProgress(BaseModel):
    ...
    courserun_start_on: datetime.datetime | None
    courserun_end_on: datetime.datetime | None = Field(description="Null for self-paced runs.")
    enrolled_on: datetime.datetime = Field(description="When this run's enrollment was created.")
    ...
    certificate_issued_on: datetime.datetime | None
    certificate_is_revoked: bool | None = Field(
        description="Null where no certificate exists. A revoked certificate leaves "
        "completion_status at `passed`."
    )
