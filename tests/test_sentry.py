...
from ol_analytics_api.core.observability.sentry import (
    _before_send,
    _scrub_pg_detail,
    _scrub_pg_details,
)
...
def sentry_transport():
    """Initialize the real SDK with _before_send, and detach it afterwards."""
...
def test_detail_line_is_truncated():
    ...
    scrubbed = _scrub_pg_detail(PG_INTEGRITY_ERROR)
...
def test_escaped_detail_line_is_truncated():
    ...
    scrubbed = _scrub_pg_detail(repr(Exception(PG_INTEGRITY_ERROR)))
...
def test_before_send_scrubs_the_event():
    ...
    assert "learner@example.invalid" not in repr(_before_send(event, {}))
...
def test_scrubs_breadcrumb_messages():
    ...
    event = {
        "breadcrumbs": {
            "values": [
                {"type": "log", "category": "django_scim.views", "message": PG_INTEGRITY_ERROR}
            ]
        }
    }
    _scrub_pg_details(event)
