"""Unit tests for the per-(schema, MV) as_of cache and its cold-cache dedup.
...
async def test_latest_refresh_timestamp_caches_after_first_call():
    ...
    with _patch_fetch_all(fetch_all):
        assert await latest_refresh_timestamp("schema_a", "mv_a") == _AS_OF
        assert await latest_refresh_timestamp("schema_a", "mv_a") == _AS_OF
...
async def test_concurrent_cold_cache_calls_dedup_to_one_query():
    ...
    the same (schema, MV) must serialize onto a single fetch_all call via
    the per-key lock, rather than each racing past the cache check."""
    _clear_cache()
    ...
    with _patch_fetch_all(fetch_all):
        tasks = [
            asyncio.create_task(latest_refresh_timestamp("schema_b", "mv_b")) for _ in range(5)
        ]
...
async def test_different_mvs_query_independently():
    _clear_cache()
    fetch_all = AsyncMock(return_value=[{"as_of": _AS_OF}])
    with _patch_fetch_all(fetch_all):
        await latest_refresh_timestamp("schema_c", "mv_c")
        await latest_refresh_timestamp("schema_c", "mv_d")
        await latest_refresh_timestamp("schema_d", "mv_c")

    assert fetch_all.await_count == 3
